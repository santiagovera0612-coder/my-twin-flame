...
function AgenteIAPage() {
  ...
  if (!agent && !creating) {
    return <EmptyState onStart={() => setCreating(true)} />;
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Paso {Math.min(steps.indexOf(step) + 1, steps.length)} de {steps.length}
              </p>
            </div>
          </div>
          <button
...
function promptFor(step: StepId, d: Partial<AgentProfile>): React.ReactNode {
  switch (step) {
    ...
    case "allowedTopics":
      return <>¿Qué cosas puede responder tu agente?</>;
    case "forbiddenClaims":
      return <>¿Qué cosas <b>no querés</b> que la IA invente?</>;
    case "escalation":
      return <>¿Cuándo debería derivar la conversación a una persona?</>;
    case "hotLeads":
      return (
        <>
          ¿Cómo querés que CLERIVO detecte <b>clientes calientes</b>? Marcá las señales típicas de
          tu negocio.
        </>
      );
    case "catalog":
    ...
    return (
      <MultiChips
        ...
        onConfirm={() => {
          ...
          else if (step === "allowedTopics")
            onAdvance({ ...draft, allowedTopics: multi }, display);
          else if (step === "forbiddenClaims")
            onAdvance({ ...draft, forbiddenClaims: multi }, display);
          else if (step === "escalation")
            onAdvance({ ...draft, escalationRules: multi }, display);
          else if (step === "hotLeads")
            onAdvance({ ...draft, hotLeadRules: multi }, display);
        }}
      />
    );
  }
      ...
      <div className="space-y-3">
        <div className="surface-card border-primary/30 p-5 shadow-elegant">
          ...
          <div className="grid gap-3 text-sm sm:grid-cols-2">
            ...
            <Row label="Puede responder" value={final.allowedTopics.join(" · ")} />
            <Row label="No debe inventar" value={final.forbiddenClaims.join(" · ")} />
            <Row label="Deriva a humano" value={final.escalationRules.join(" · ")} />
            <Row label="Clientes calientes" value={final.hotLeadRules.join(" · ")} />
            <Row
              label="Catálogo"
              value={final.catalogEnabled ? "Cargar ahora" : "Cargar después"}
            />
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
...
function ProfilePanel({
  ...
  const modes: { v: AgentProfile["operatingMode"]; label: string; desc: string }[] = [
    { v: "suggest", label: "Sugerir", desc: "Solo sugiere respuestas" },
    { v: "approve", label: "Aprobar", desc: "Responde con aprobación (recomendado)" },
    { v: "auto", label: "Automático", desc: "Responde automáticamente" },
  ];
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          {modes.map((m) => {
            ...
            return (
              <button
                key={m.v}
                onClick={() => update("operatingMode", m.v)}
